...
SELECT p.id AS pedido_id, u.nome AS usuario, p.status, p.total
FROM pedidos p
JOIN usuarios u ON p.usuario_id = u.id;

-- 2. Listar produtos de cada pedido (com nome do produto, quantidade e valor unitário)
SELECT p.id AS pedido_id, pr.nome AS produto, ip.quantidade, pr.preco AS preco_unitario, 
...
SELECT u.nome AS usuario, pg.valor_pagamento, pg.metodos_pagamento, pg.status
FROM pagamentos pg
...
SELECT u.nome AS usuario, p.id AS pedido_id, ee.endereco, ee.cidade,
ee.estado, ee.caixa_postal, ee.pais
...
SELECT pr.nome AS produto, pr.descricao, pr.preco, pr.quantidade_estoque,
c.nome AS categoria
FROM produtos pr
JOIN categorias c ON pr.categoria_id = c.id;

-- 6. Listar total gasto por cada usuário em pedidos já entregues
SELECT u.nome AS usuario, SUM(p.total) AS total_gasto
FROM pedidos p
JOIN usuarios u ON p.usuario_id = u.id
